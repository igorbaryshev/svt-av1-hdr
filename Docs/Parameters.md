...
| **FilmGrainCrop**                | --film-grain-crop      | [0-100:.:.:.]  | [100:100] | Crop film grain noise estimation area, default is full frame [<width_percent>:<height_percent>]. Optionally can set X and Y offsets for the crop area, by default those offsets are equal to `(100 - Crop) / 2`(centered crop area).<br>`Width + X` and `height + Y` cannot exceed 100.<br> *Example*: `80:80:10:10`, which is equivalent to `80:80`. <br> One block will be used at minimum for estimation.|
