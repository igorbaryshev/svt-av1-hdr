...
static void set_param_based_on_input(SequenceControlSet *scs)
{
    ...
    if (scs->static_config.photon_noise_iso > 0) {
        ...
        if (scs->static_config.fgs_table) {
            ...
            scs->static_config.photon_noise_chroma = -1;
        } else {
        ...
        if (scs->static_config.photon_noise_chroma != -1) {
...
static void copy_api_from_app(SequenceControlSet *scs, EbSvtAv1EncConfiguration *config_struct) {
    ...
    scs->static_config.photon_noise_chroma = config_struct->photon_noise_chroma;
    ...
    if (scs->static_config.tune == TUNE_FILM_GRAIN) {
        ...
        SVT_WARN("Tune 5: Film Grain turns off: TF, CDEF, rest. filtering, and enables complex HVS, TX bias and strong AC bias.\n");
...
EB_API void svt_av1_print_version(void) {
    SVT_INFO("-------------------------------------------\n");
    SVT_INFO("SVT [version]:\tSVT-AV1-HDR Encoder Lib %s \"Saint-14\"\n", SVT_AV1_CVS_VERSION);
    const char *compiler =
